"""Ponto de entrada: ``python -m mousebridge {server|client|gui}``."""
...
from mousebridge import __version__
from mousebridge.config import ConfigError, load_config
from mousebridge.config.settings import VALID_EDGES
from mousebridge.logging_setup import setup_logging

logger = logging.getLogger(__name__)


def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(prog="mousebridge", description="Compartilhamento de mouse e teclado em rede local")
    parser.add_argument("--version", action="version", version=f"mousebridge {__version__}")
...
def main(argv: list[str] | None = None) -> int:
    ...
    if args.command == "gui":
        from mousebridge.gui import run_gui
    ...
    if args.command == "server":
        from mousebridge.server import MouseBridgeServer

        core: object = MouseBridgeServer(config)
    else:
        from mousebridge.client import MouseBridgeClient

        core = MouseBridgeClient(config)
