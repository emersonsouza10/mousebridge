"""Interface gráfica do MouseBridge (Tkinter).
...
class _CoreThread(threading.Thread):
    ...
    def __init__(self, config: AppConfig, status_queue: "queue.Queue[dict[str, Any]]") -> None:
        super().__init__(daemon=True, name="mousebridge-core")
        self._config = config
    ...
    def run(self) -> None:
        ...
        if self._config.role == "server":
            self._core = MouseBridgeServer(self._config, on_status=on_status)
        else:
            self._core = MouseBridgeClient(self._config, on_status=on_status)
        try:
